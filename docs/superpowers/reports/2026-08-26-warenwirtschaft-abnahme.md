...
vorhandenen `allocated_purchase_cost` als COGS-Snapshot. Titel und Steuerart
werden aus den unveränderten historischen Datensätzen abgeleitet.
...
Die SQL-Verträge liefen lokal gegen den Supabase-Postgres-Container. Eine
interaktive Browserprüfung war in dieser Ausführungsumgebung nicht verfügbar;
die sichtbaren Werte wurden deshalb über dieselben persistenten Tabellen und
RPC-Ergebnisse geprüft, die Inventar, Verkauf, Dashboard und Shop versorgen.
...
| `Get-Content -Raw supabase/tests/inventory_sales_legacy_migration.sql \| docker exec ... psql ...`       | PASS: `BEGIN`, `DO`, `ROLLBACK`.                                                                         |
| Vormigrationsfixture + tatsächliche `20260828101500_backfill_legacy_sale_lines.sql` im lokalen Container | PASS: sechs Inserts, `DO`, `ROLLBACK`.                                                                   |
| Zentraler Mengen-/Einzelverkauf-/Shop-Checkoutvertrag im lokalen Container                               | PASS: `BEGIN`, `DO`, `ROLLBACK`.                                                                         |
| `npm run typecheck`                                                                                      | PASS.                                                                                                    |
| `npm test -- --run`                                                                                      | Vorbestehender Fehlschlag: 674/685 Tests bestanden; 11 Fehler in drei unabhängigen UI-/Persistenzsuiten. |
...
- Der einmalige Gesamtlauf meldet 11 Fehler in
  `sales-persistence-actions.spec.ts`, `sales-toast-actions.spec.ts` und
  `purchase-create-modal-actions.spec.ts`. Die Fehler betreffen ältere Mocks,
  die nicht mehr zu bereits bestehenden Signal-/Service-Schnittstellen passen;
  Task 9 ändert keine dieser Dateien. Sie wurden daher nicht außerhalb des
  Abnahmeumfangs umgebaut.
