...
- [ ] Fix observed existing light-mode contrast failure in `features/purchases/pages/purchase-detail/purchase-detail.component.ts` purchase-type badge (`bg-fb-art-single/10 ... text-fb-art-single`, line197 at preflight). Use readable neutral/accent text while preserving dark surfaces. Regression/axe against full purchase detail main, not just picker.
