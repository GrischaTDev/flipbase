...
- Alle 15 betroffenen Feature-Templates wurden vollständig auf die Shared Controls migriert: native `<input>`- und `<textarea>`-Felder durch `app-text-field` und `app-number-input`, native `<input type="checkbox">` durch `app-custom-checkbox`, native Custom-Dialoge durch `app-modal-shell` und Aktionsschaltflächen durch `app-button`.
