...
gelesen. Die neue Release-Migration bricht vor jeder Änderung mit SQLSTATE `55000`
ab, falls nach dieser Prüfung doch wieder ein alter Entwurf entstanden ist. Das
deklarative Schema, die Archivierungsregistrierung und die generierten Typen
enthalten die alte Tabelle nicht mehr. Im Frontend bleibt nur der tatsächlich
verwendete Kleinanzeigen-Textgenerator als kleiner Dienst im Inserate-Feature;
unbenutzte eBay-, Vinted-, Webshop-, HTML-, SEO- und Direktveröffentlichungswege
sind entfernt.

**Prüfung:** Der Migrationsschutz wurde in einer Wegwerf-Datenbank mit beiden
Ausgängen geprüft: Ein vorhandener Datensatz stoppte die Migration und blieb
erhalten, eine leere Tabelle wurde entfernt. Der Datenbank-Reset, 62 gezielte
Inserate-, 100 Archivierungs- und alle 1.937 Datenbankprüfungen bestanden.
`npm run verify` war mit 1.428 Node-, 239 DOM-, 940 Angular- und 13 Landing-Tests
sowie Format, ESLint, Typprüfung, Workflow-Prüfungen, Suite-Audit und
Produktionsbau grün. Alle sieben PR-Chromium-Abläufe bestanden. Der Bau meldet
weiterhin die drei bekannten NG8113-Hinweise außerhalb des Inserate-Bereichs;
der unveränderte Steuerjournal-Ablauf meldet weiterhin den bestehenden
NG0956-Laufzeithinweis. Kein Push und kein Merge.
