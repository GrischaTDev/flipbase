...
Das ist genau der Punkt, den Plan-Aufgabe **5.2.3** verlangt hat: *„Leere `catch {}` durch echte Fehlerbehandlung ersetzen: Signal `syncError`, sichtbarer Hinweis in der Oberfläche."* Ein solches Signal existiert nirgends. Für ein Werkzeug, das Einkäufe und Buchhaltung führt, ist ein stillschweigend verlorener Datensatz der schlimmste Ausgang.
...
Damit bringt der typisierte Client an dieser Stelle nichts. Deine `CLAUDE.md` verlangt ausdrücklich: *„`any` vermeiden; `unknown` verwenden, wenn der Typ unsicher ist."*
