-- Interne Chronik: neue Tabelle record_comments und lesende list_record_timeline-RPC.
-- Schema automatisch mit Supabase pg-delta erzeugt; ACL aus der Deklaration übernommen.
...
create function public.list_record_timeline (
  p_workspace_id      uuid,
  p_entity_type       text,
  p_entity_id         uuid,
...
comment on function public.list_record_timeline(uuid,text,uuid,timestamp with time zone,text,uuid,integer) is 'Datensatzbezogene Chronik nach Mitgliedschafts- und Existenzprüfung mit stabiler gemeinsamer Paginierung.';

revoke all on function public.list_record_timeline(uuid, text, uuid, timestamp with time zone, text, uuid, integer) from public;

grant all on function public.list_record_timeline(uuid, text, uuid, timestamp with time zone, text, uuid, integer) to authenticated;
...
comment on table public.record_comments is 'Unveränderliche interne Klartext-Kommentare zur gemeinsamen Einkaufs- und Verkaufschronik.';

alter table public.record_comments
  enable row level security;

alter table public.record_comments
  add constraint record_comments_author_id_fkey foreign key (author_id) references public.profiles(id) on delete restrict;

alter table public.record_comments
  add constraint record_comments_body_length
    check (body = btrim(body, ((((chr(32) || chr(9)) || chr(10)) || chr(13)) || chr(12)) || chr(11)) and char_length(body) >= 1 and char_length(body) <= 5000);

alter table public.record_comments
  add constraint record_comments_one_entity check (num_nonnulls(purchase_id, sale_id) = 1);

alter table public.record_comments
  add constraint record_comments_pkey primary key (id);

alter table public.record_comments
  add constraint record_comments_purchase_fkey foreign key (workspace_id, purchase_id) references public.purchases(workspace_id, id) on delete restrict;

alter table public.record_comments
  add constraint record_comments_sale_fkey foreign key (workspace_id, sale_id) references public.sales(workspace_id, id) on delete restrict;

alter table public.record_comments
  add constraint record_comments_workspace_id_fkey foreign key (workspace_id) references public.workspaces(id) on delete restrict;

grant insert (author_id, body, purchase_id, sale_id, workspace_id) on public.record_comments to authenticated;
...
create trigger "00_protect_archived_workspace"
  before insert or delete or update on public.record_comments
  for each row
...
create policy "Mitglieder lesen Kommentare" on public.record_comments
  for select
  to authenticated
  using ((( select public.is_workspace_member(record_comments.workspace_id) as is_workspace_member) and ((exists ( select 1
...
create policy "Mitglieder schreiben eigene Kommentare" on public.record_comments
  for insert
  to authenticated
  with check (((author_id = ( select auth.uid() as uid)) and ( select public.is_workspace_member(record_comments.workspace_id) as is_workspace_member) and ((exists ( select 1
   from public.purchases p
  where ((p.workspace_id = record_comments.workspace_id) and (p.id = record_comments.purchase_id)))) or (exists ( select 1
   from public.sales s
  where ((s.workspace_id = record_comments.workspace_id) and (s.id = record_comments.sale_id)))))));
-- Explizite Freigaben gelten unabhängig von bestehenden Server-Defaults.
