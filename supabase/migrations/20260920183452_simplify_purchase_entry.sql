-- Vereinfacht die Einkaufserfassung und entfernt die nicht mehr verwendeten
-- Spalten purchases.original_url, external_order_id und seller_marketplace_username.
...
ALTER TABLE public.purchases
  DROP CONSTRAINT purchases_external_order_id_length_check;

ALTER TABLE public.purchases
  DROP COLUMN external_order_id;

ALTER TABLE public.purchases
  DROP CONSTRAINT purchases_seller_marketplace_username_length_check;

ALTER TABLE public.purchases
  DROP COLUMN seller_marketplace_username;
...
CREATE OR REPLACE FUNCTION public.normalize_purchase_seller_details (
  p_details jsonb
)
  RETURNS jsonb
  LANGUAGE sql
...
CREATE OR REPLACE FUNCTION public.purchase_seller_details_snapshot (
  p_purchase public.purchases
)
  RETURNS jsonb
  LANGUAGE sql
  IMMUTABLE
...
COMMENT ON FUNCTION public.update_purchase_seller_details(uuid,uuid,integer,jsonb,text) IS 'Trägt Quelle, Verkäufer-Snapshot und Referenz nach, auch bei abgeschlossenen Einkäufen. Ändert keine Kosten, Positionen, Bestände oder Status.';
