...
CREATE FUNCTION public.prevent_open_price_purchase_stock_transfer()
  RETURNS TRIGGER
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO ''
...
REVOKE ALL ON FUNCTION public.prevent_open_price_purchase_stock_transfer() FROM PUBLIC;

CREATE FUNCTION public.purchase_has_open_prices (
  p_workspace_id uuid,
  p_purchase_id  uuid
)
  RETURNS boolean
...
COMMENT ON FUNCTION public.purchase_has_open_prices(uuid,uuid) IS 'Erkennt offene Preise in normalen Einkaufspositionen, einschließlich alter unpriced_mystery-Daten.';

REVOKE ALL ON FUNCTION public.purchase_has_open_prices(uuid, uuid) FROM PUBLIC;
...
CREATE TRIGGER prevent_open_price_purchase_inventory_transfer
  ...
  FOR EACH ROW
  EXECUTE FUNCTION public.prevent_open_price_purchase_stock_transfer();

ALTER TABLE public.purchase_lines
  ADD CONSTRAINT purchase_lines_check2 CHECK (price_mode = 'priced'::text AND unit_purchase_price IS NOT NULL AND line_total IS
    NOT NULL AND line_total = round(ordered_quantity::numeric * unit_purchase_price, 2) OR (price_mode = ANY (ARRAY['open'::text, 'unpriced_mystery'::text])) AND
    unit_purchase_price IS NULL AND line_total IS NULL);

ALTER TABLE public.purchase_lines
  ADD CONSTRAINT purchase_lines_price_mode_check CHECK (price_mode = ANY (ARRAY['priced'::text, 'open'::text, 'unpriced_mystery'::text]));
