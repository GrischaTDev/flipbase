-- migration: 20260918162500_expenses_foundation.sql
-- purpose: persist operating expenses, recurring rules, workspace categories and private expense documents

-- Betriebsausgaben, wiederkehrende Kosten und private Ausgabenbelege.
-- Wareneinkäufe bleiben im bestehenden Purchase-Modell.
...
create unique index if not exists expense_categories_active_name_uq
  on public.expense_categories (workspace_id, lower(pg_catalog.btrim(name)))
  where is_archived = false;

create index if not exists expense_categories_workspace_sort_idx
  on public.expense_categories (workspace_id, is_archived, sort_order, name);
...
create index if not exists expense_recurring_rules_workspace_active_idx
  on public.expense_recurring_rules (workspace_id, is_active, start_date, id);
...
create index if not exists expenses_workspace_payment_idx
  on public.expenses (workspace_id, payment_date desc, id)
  where status = 'paid' and deleted_at is null;
create index if not exists expenses_workspace_status_idx
  on public.expenses (workspace_id, status, due_date, id)
...
drop trigger if exists touch_expense_categories_updated_at on public.expense_categories;
...
drop trigger if exists touch_expense_recurring_rules_updated_at on public.expense_recurring_rules;
create trigger touch_expense_recurring_rules_updated_at
...
create or replace function public.seed_default_expense_categories_after_workspace()
returns trigger
language plpgsql
security definer
set search_path = ''
...
alter function public.seed_default_expense_categories_after_workspace() owner to postgres;
revoke all on function public.seed_default_expense_categories_after_workspace()
  from public, anon, authenticated, service_role;

drop trigger if exists seed_default_expense_categories_after_workspace on public.workspaces;
create trigger seed_default_expense_categories_after_workspace
  after insert on public.workspaces
  for each row execute function public.seed_default_expense_categories_after_workspace();
...
revoke all on table public.expense_recurring_rules from public, anon, authenticated, service_role;
...
create policy "Wiederkehrende Ausgaben lesen" on public.expense_recurring_rules
  for select to authenticated
  using ((select public.is_workspace_member(workspace_id)));

create policy "Wiederkehrende Ausgaben anlegen" on public.expense_recurring_rules
  for insert to authenticated
  with check (
    (select public.is_workspace_member(workspace_id))
    and created_by = (select auth.uid())
...
create policy "Wiederkehrende Ausgaben aendern" on public.expense_recurring_rules
  for update to authenticated
  using ((select public.is_workspace_member(workspace_id)))
  with check ((select public.is_workspace_member(workspace_id)));
...
revoke all on table public.expense_documents from public, anon, authenticated, service_role;
...
create policy "Ausgabenbelege lesen" on storage.objects
  for select to authenticated
  using (
...
create policy "Ausgabenbelege hochladen" on storage.objects
  for insert to authenticated
  with check (
    bucket_id = 'expense-documents'
    and exists (
...
create policy "Ausgabenbelege entfernen" on storage.objects
  for delete to authenticated
  using (
    bucket_id = 'expense-documents'
    and exists (
