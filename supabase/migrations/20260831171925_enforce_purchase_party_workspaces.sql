-- Purpose: keep purchase sources and suppliers inside the purchase workspace.
-- Affected tables: public.sources, public.suppliers, public.purchases.
...
begin
  if exists (
    ...
    join public.sources as source on source.id = purchase.source_id
    where source.workspace_id <> purchase.workspace_id
  ) then
    raise exception 'Bestehende Einkäufe enthalten Quellen aus einem fremden Workspace.';
  end if;

  if exists (
    select 1
    from public.purchases as purchase
    join public.suppliers as supplier on supplier.id = purchase.supplier_id
    where supplier.workspace_id <> purchase.workspace_id
  ) then
    raise exception 'Bestehende Einkäufe enthalten Lieferanten aus einem fremden Workspace.';
  end if;
end;
$$;

ALTER TABLE public.purchases
  DROP CONSTRAINT purchases_source_id_fkey;
...
ALTER TABLE public.purchases
  ADD CONSTRAINT purchases_workspace_source_fkey FOREIGN KEY (workspace_id, source_id) REFERENCES public.sources(workspace_id, id) ON DELETE SET NULL (source_id);

ALTER TABLE public.suppliers
  ADD CONSTRAINT suppliers_workspace_id_id_key UNIQUE (workspace_id, id);

ALTER TABLE public.purchases
  ADD CONSTRAINT purchases_workspace_supplier_fkey FOREIGN KEY (workspace_id, supplier_id) REFERENCES public.suppliers(workspace_id, id) ON DELETE SET NULL (supplier_id);
