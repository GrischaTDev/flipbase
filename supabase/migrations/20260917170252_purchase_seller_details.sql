-- migration: 20260917170252_purchase_seller_details.sql
-- purpose: store a seller snapshot per purchase and allow amending origin details,
--   also for finalized purchases, without touching costs, lines or stock
-- affected tables: purchases (seller_type, seller_name, seller_marketplace_username,
--   seller_street, seller_address_extra, seller_postal_code, seller_city,
--   seller_country_code, external_order_id, seller_details_version)
-- affected functions: normalize_purchase_seller_details, purchase_seller_details_snapshot,
--   update_purchase_seller_details (new); purchase_draft_audit_snapshot, create_purchase,
--   update_purchase_draft (replaced; bodies taken from supabase/schemas/database.sql)
-- note: written from the declarative schema files because `supabase db diff` currently
--   fails on the existing schema order (50_sniper.sql uses is_platform_operator from
--   99_platform_admin.sql). create or replace keeps existing owners and grants.
