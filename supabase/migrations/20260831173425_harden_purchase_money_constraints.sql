-- Purpose: reject invalid and sub-cent purchase money before PostgreSQL can round it.
-- Affected columns: public.purchases.purchase_price, public.purchase_costs.amount.
...
begin
  if exists (
    ...
    where purchase.purchase_price is not null
      and (
        purchase.purchase_price = 'NaN'::numeric
        or purchase.purchase_price < 0
        or purchase.purchase_price <> pg_catalog.round(purchase.purchase_price, 2)
      )
  ) then
    raise exception 'Bestehende Einkäufe enthalten ungültige oder nicht centgenaue Kaufpreise.';
  end if;

  if exists (
    select 1
    from public.purchase_costs as cost
    where cost.amount = 'NaN'::numeric
      or cost.amount <= 0
      or cost.amount <> pg_catalog.round(cost.amount, 2)
  ) then
    raise exception 'Bestehende Einkäufe enthalten ungültige oder nicht centgenaue Zusatzkosten.';
  end if;
end;
$$;

-- Values such as 10.0000000000000000 can come from earlier integer-cent
-- calculations. The preflight above proves that reducing only their stored
-- scale cannot change their value.
update public.purchases
set purchase_price = pg_catalog.round(purchase_price, 2)
where purchase_price is not null
  and pg_catalog.scale(purchase_price) > 2;

update public.purchase_costs
set amount = pg_catalog.round(amount, 2)
where pg_catalog.scale(amount) > 2;
...
ALTER TABLE public.purchase_costs
  ADD CONSTRAINT purchase_costs_amount_check CHECK (amount <> 'NaN'::numeric AND amount > 0::numeric AND scale(amount) <= 2);

ALTER TABLE public.purchases
  ADD CONSTRAINT purchases_purchase_price_check CHECK (purchase_price IS NULL OR purchase_price <> 'NaN'::numeric AND purchase_price >= 0::numeric AND scale(purchase_price) <= 2);
