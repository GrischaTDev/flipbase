...
CREATE FUNCTION public.export_audit_snapshot (
  p_workspace_id uuid,
...
COMMENT ON FUNCTION public.export_audit_snapshot(uuid,jsonb) IS 'Vollständige Archivtabellen aus einem gemeinsamen STABLE-Snapshot für Owner/Admin/Accountant; maximal 100000 Zeilen und 50 MiB JSON, sonst expliziter Fehler.';

REVOKE ALL ON FUNCTION public.export_audit_snapshot(uuid, jsonb) FROM PUBLIC;

GRANT ALL ON FUNCTION public.export_audit_snapshot(uuid, jsonb) TO authenticated;

GRANT ALL ON FUNCTION public.export_audit_snapshot(uuid, jsonb) TO service_role;
