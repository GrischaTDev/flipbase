-- Archivierung ändert ausschließlich Metadaten; Verkauf und Buchungen bleiben erhalten.
...
create or replace function public.protect_inventory_archive_metadata()
...
$$;
create trigger protect_inventory_archive_metadata before insert or update on public.inventory_items
for each row execute function public.protect_inventory_archive_metadata();
revoke execute on function public.protect_inventory_archive_metadata() from public, anon, authenticated, service_role;

-- SECURITY DEFINER überwindet ausschließlich bestehende Finalisierungssperren.
-- Mitgliedschaft, Workspace-Zustand und exakter Artikel werden vorher gesperrt/geprüft.
