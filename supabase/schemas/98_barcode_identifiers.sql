-- EAN/GTIN remains optional, but supplied purchase identifiers are validated server-side.
...
alter table public.purchase_lines
  drop constraint if exists purchase_lines_ean_snapshot_check;

alter table public.purchase_lines
  add constraint purchase_lines_ean_snapshot_check
...
begin
  if not exists (
    ...
    where tgrelid = 'public.inventory_items'::pg_catalog.regclass
      and tgname = 'sync_inventory_item_ean'
  ) then
    create trigger sync_inventory_item_ean
      before insert or update of purchase_line_id, ean on public.inventory_items
