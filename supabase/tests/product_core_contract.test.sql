...
reset role;
create temporary table contract_receipts as select to_jsonb(lot) - array['unit_cost', 'unit_tax_purchase_cost', 'remaining_unit_costs', 'remaining_tax_unit_costs']::text[] as lot, (select jsonb_agg(to_jsonb(movement) order by movement.id) from public.stock_movements as movement where movement.stock_lot_id = lot.id) as movements from public.stock_lots as lot where lot.purchase_id = (select id from contract_purchases where label = 'main');
...
select is((select count(*) from public.stock_lots where purchase_id = (select id from contract_purchases where label = 'main') and unit_tax_purchase_cost is null), 3::bigint, 'Wiederöffnen setzt den steuerlichen Stückpreis aller Lose ausdrücklich auf NULL');
select is((select count(*) from public.stock_lots where purchase_id = (select id from contract_purchases where label = 'main') and remaining_unit_costs is null), 3::bigint, 'Wiederöffnen setzt die betrieblichen Stückkostenfolgen aller Lose ausdrücklich auf NULL');
select is((select count(*) from public.stock_lots where purchase_id = (select id from contract_purchases where label = 'main') and remaining_tax_unit_costs is null), 3::bigint, 'Wiederöffnen setzt die steuerlichen Stückkostenfolgen aller Lose ausdrücklich auf NULL');
select results_eq($$select to_jsonb(lot) - array['unit_cost', 'unit_tax_purchase_cost', 'remaining_unit_costs', 'remaining_tax_unit_costs']::text[], (select jsonb_agg(to_jsonb(movement) order by movement.id) from public.stock_movements as movement where movement.stock_lot_id = lot.id) from public.stock_lots as lot where lot.purchase_id = (select id from contract_purchases where label = 'main') order by lot.id$$, $$select lot, movements from contract_receipts order by lot ->> 'id'$$, 'Wiederöffnen bewahrt Los-IDs, Mengen, Zeitpunkte und vollständige Empfangshistorie');
