  ...
  describe('Aktionsmeldungen', () => {
    ...
    describe('SettingsComponent – zentrale Aktionsmeldungen', () => {
      ...
      it.each([
        ...
        [
          'Versanddienstleister wurden gespeichert.',
          (c: SettingsComponent) => c.onSaveCarrierConfig(),
        ],
        [
          'Webhook-Konfiguration wurde gespeichert.',
          (c: SettingsComponent) => c.onSaveWebhookConfig(),
        ],
      ])(
      ...
      it('setzt Ladezustände auch bei geworfenen Konfigurationsfehlern zurück', async () => {
        const { komponente, toast } = erstelleKomponente();
        vi.mocked(komponente.storeService.updatePaymentsConfig).mockRejectedValue(
          new Error('offline'),
        );
      ...
      it('meldet das Ergebnis der Browser-Berechtigung als Erfolg oder angepinnten Fehler', async () => {
        ...
        erwarteEinzelnenToast(
          erfolg.toast,
          'success',
          'Browser-Benachrichtigungen wurden aktiviert.',
        );
