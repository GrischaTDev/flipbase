...
import { SchluesselWarteschlange } from './async-warteschlange';
...
describe('Schluesselbasierte Async-Warteschlange', () => {
  it('zaehlt eingereihte Aufgaben sofort als ausstehend', () => {
    const warteschlange = new SchluesselWarteschlange<string>();
    const freigabe = aufschiebbar<void>();

    void warteschlange.einreihen('bild-1', () => freigabe.promise);
    void warteschlange.einreihen('bild-1', () => freigabe.promise);

    expect(warteschlange.anzahlAusstehend()).toBe(2);
  });

  it('zaehlt eine erfolgreiche Aufgabe nach ihrem Abschluss herunter', async () => {
    const warteschlange = new SchluesselWarteschlange<string>();

    await warteschlange.einreihen('bild-1', async () => 'fertig');

    expect(warteschlange.anzahlAusstehend()).toBe(0);
  });
  ...
  it('zaehlt auch eine fehlgeschlagene Aufgabe nach ihrem Abschluss herunter', async () => {
    ...
    await expect(
      warteschlange.einreihen('bild-1', async () => {
  ...
  it('startet Aufgaben derselben ID nacheinander', async () => {
    ...
    const erste = warteschlange.einreihen('bild-1', async () => {
      gestartet.push('erste');
      await ersteFreigabe.promise;
      return 1;
    });
    const zweite = warteschlange.einreihen('bild-1', async () => {
  ...
  it('laesst Aufgaben verschiedener IDs parallel starten', async () => {
    ...
    const zweite = warteschlange.einreihen('bild-2', async () => {
  ...
  it('fuehrt die naechste Aufgabe auch nach einem Fehler aus', async () => {
    ...
    const fehlgeschlagen = warteschlange.einreihen('bild-1', async () => {
      reihenfolge.push('fehler');
      throw new Error('kaputt');
    });
    const danach = warteschlange.einreihen('bild-1', async () => {
