...
export class ExpenseDocumentService {
  ...
  async loadSummaryForExpenses(expenseIds: readonly string[]): Promise<void> {
    ...
    if (this.mockStore.isDemoMode() || !workspace || uniqueIds.length === 0) {
    ...
    try {
      ...
      this.documentCounts.set(new Map(uniqueIds.map((id) => [id, 0])));
    }
  }
  ...
  async upload(
    ...
    try {
      ...
      this.setDocumentCount(expenseId, (this.documentCounts().get(expenseId) ?? 0) + 1);
  ...
  async remove(document: ExpenseDocument): Promise<{ error: Error | null }> {
    ...
    try {
      ...
      this.setDocumentCount(
        document.expense_id,
        Math.max(0, (this.documentCounts().get(document.expense_id) ?? 1) - 1),
      );
      if (cleanup.error) {
        return {
  ...
  private setDocumentCount(expenseId: string, count: number): void {
    this.documentCounts.update((current) => {
      ...
      next.set(expenseId, Math.max(0, count));
      return next;
    });
  }
}
